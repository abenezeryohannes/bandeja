<template>
    <div class="card">
        <div class="card-header" :class="card_class(the_data.status)">
            <h3 class="text-white mb-0 float-right">{{the_data.status}}</h3>
        </div>
        <div class="card-body">
            <div class="row mx--4">
                <div class="col-md-7 border-bottom-1">
                    <div class="table-responsive mt-2">
                        <table class="table table-borderless">
                            <tbody>
                                <tr>
                                    <th><img src="data:image/png;base64," alt="My Company"></th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="col-md-5 border-bottom-1">
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            <tbody>
                                <tr>
                                    <th> Walta Eth </th>
                                </tr>
                                <tr>
                                    <th> Ethiopia, Addis Ababa </th>
                                </tr>
                                <tr>
                                    <th>Tax No: 35 </th>
                                </tr>
                                <tr>
                                    <th> my@company.com </th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-7 long-texts">
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            <tbody>
                                <tr>
                                    <th> Bill To <strong class="d-block">{{the_data.tenant.name}}</strong></th>
                                </tr>
                                <tr>
                                    <th> {{the_data.tenant.physical_address}}</th>
                                </tr>
                                <tr>
                                    <th> Tin Numbers: {{the_data.tenant.tin_number}} </th>
                                </tr>
                                <tr>
                                    <th> {{the_data.tenant.phone_number}} </th>
                                </tr>
                                <tr>
                                    <th> {{the_data.tenant.email_address}} </th>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="col-md-5 long-texts">
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            <tbody>
                                <tr>
                                    <th>Invoice Number:</th>
                                    <td class="text-right">{{the_data.invoice_number}}</td>
                                </tr>
                                <tr>
                                    <th>Invoice Date:</th>
                                    <td class="text-right">{{the_data.invoice_date}}</td>
                                </tr>
                                <tr>
                                    <th>Payment Due:</th>
                                    <td class="text-right">{{the_data.due_date}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
            <div class="row show-table">
                <div class="col-md-12">
                    <div class="table-responsive overflow-y-hidden">
                        <table class="table table-striped">
                            <tbody>
                                <tr class="d-flex flex-nowrap">
                                    <th class="col-sm-3 pl-5">Property</th>
                                    <th class="col-sm-2 text-center">From</th>
                                    <th class="col-sm-2 text-right d-none d-sm-block">Period</th>
                                    <th class="col-sm-2 text-center">Price</th>
                                    <th class="col-sm-3 text-right pr-5">Total</th>
                                </tr>
                                <tr class="d-flex flex-nowrap">
                                    <td class="col-sm-3 pl-5">{{the_data.property.name}}</td>
                                    <td class="col-sm-2 text-center">{{the_data.due_date}}</td>
                                    <td class="col-sm-2 text-right d-none d-sm-block">{{the_data.period}}</td>
                                    <td class="col-sm-2 text-center">{{the_data.price}}</td>
                                    <td class="col-sm-3 text-right pr-5">{{the_data.price}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
            <div class="row mt-5">
                <div class="col-md-7">
                    <!-- <div class="table-responsive">
                        <table class="table table-borderless">
                            <tbody>
                                <tr>
                                    <th>
                                        <p class="form-control-label">Notes</p>
                                        <p class="text-muted long-texts">Qui.</p>
                                    </th>
                                </tr>
                            </tbody>
                        </table>
                    </div> -->
                </div>
                <div class="col-md-5">
                    <div class="table-responsive">
                        <table class="table">
                            <tbody>
                                <tr>
                                    <th>Subtotal:</th>
                                    <td class="text-right">{{the_data.subtotal}}</td>
                                </tr>
                                <tr>
                                    <th>Tax:</th>
                                    <td class="text-right">{{the_data.tax_price}}</td>
                                </tr>
                                <tr>
                                    <th>Total:</th>
                                    <td class="text-right">{{the_data.total}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
        <div class="card-footer">
            <div class="row align-items-center">
                <div class="col-xs-12 col-sm-4"></div>
                <div class="col-xs-12 col-sm-8 text-right">
                    

                    <a href="http://localhost:8001/sales/invoices/28/edit" class="btn btn-info header-button-top"><i class="fas fa-edit"></i>&nbsp; Edit </a>
                    <!-- <a href="http://localhost:8001/sales/invoices/28/print" target="_blank" class="btn btn-success header-button-top"><i class="fa fa-print"></i>&nbsp; Print </a> -->
                    <a href="http://localhost:8001/signed/invoices/28?company_id=1&amp;signature=feb670c679bd8b1aceba075099af53e8073cea4c7028bf4e48fb5cb3750e3575" target="_blank" class="btn btn-white header-button-top"><i class="fa fa-share"></i>&nbsp; Share </a>
                    <ak-dropup>
                        <router-link  :to="{ name : 'InvoicesPay', params: { id: the_data.invoice_id } }" class="dropdown-item" >Pay</router-link>
                    	<!-- <button id="button-payment" class="dropdown-item">Pay</button> -->
                    	<a  class="dropdown-item">Cancel</a> 
                    </ak-dropup>
                    
                    <!-- <div class="dropup header-drop-top"><button type="button" data-toggle="dropdown" aria-expanded="false" class="btn btn-primary header-button-top"><i class="fa fa-chevron-up"></i>&nbsp; More Actions</button>
                        <div role="menu" class="dropdown-menu">
                        	<a href="http://localhost:8001/sales/invoices/28/paid" class="dropdown-item">Mark Paid</a> <button id="button-payment" class="dropdown-item">Add Payment</button>
                            <div class="dropdown-divider"></div> <button type="button" disabled="disabled" class="dropdown-item"><span class="text-disabled">Mark Sent</span></button> <a href="http://localhost:8001/sales/invoices/28/email" class="dropdown-item">Send Email</a> <a href="http://localhost:8001/sales/invoices/28/pdf" class="dropdown-item">Download PDF</a> <a href="http://localhost:8001/sales/invoices/28/cancelled" class="dropdown-item">Cancel</a> <button type="button" title="Delete" class="dropdown-item action-delete">Delete</button>
                        </div>
                    </div> -->

                </div>
            </div>
        </div>
    </div>
</template>
<script>
export default {

    props: ['the_data'],


    data() {
        return {

        }
    },

    methods: {
        card_class(t){
            switch(t){
                case 'Cancelled':
                    return "status-danger";
                    break;
                case 'Waiting':
                    return "status-info";
                    break;
                default:
                    return "status-success";
                    break;
            }
            return "status-danger"
            return "status-success"
        }
    }
}

</script>
