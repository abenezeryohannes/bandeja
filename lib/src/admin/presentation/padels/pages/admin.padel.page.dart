...
import 'admin.padel.page.body.dart';

class AdminPadelPage extends StatefulWidget {
  const AdminPadelPage({Key? key, required this.padel}) : super(key: key);
  final PadelModel padel;
  @override
  State<AdminPadelPage> createState() => _AdminPadelPageState();
}

class _AdminPadelPageState extends State<AdminPadelPage> {
  bool isFloating = true;
  double offset = 0;
  final ScrollController _controller = ScrollController();
  late AdminPadelPageController controller;
  final padelRepository = getIt<IPadelRepository>();

  @override
  void initState() {
    controller = Get.put(AdminPadelPageController(padel: widget.padel));
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        Get.back<String?>(
            result: (controller.approved.value != widget.padel.approved)
                ? 'saved'
                : null);
        return false;
      },
