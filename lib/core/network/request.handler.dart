...
class RequestHandler {
  ...
  static Future<Response<dynamic>?> get(
      {required path, header, query, required auth}) async {
    try {
      var dio = Dio();
      // Add the interceptor
      ...
      //404
      // print(
      //     "uri: ${Uri.parse(baseUrl + path).toString()} interceptors: ${dio.interceptors.length}");
  ...
  static Future<Response<dynamic>?> getRaw(
      {required path, header, query, required auth}) async {
    try {
      var dio = Dio();
      // Add the interceptor
      ...
      dio.options.headers["Authorization"] = "$auth";
      dio.options.headers["authorization"] = "$auth";
      return await dio.get(Uri.parse(path).toString(),
          queryParameters: query, options: Options(receiveTimeout: 5000));
    } on DioError catch (e) {
      // The request was made and the server responded with a status code
      // that falls out of the range of 2xx and is also not 304.
      if (e.response != null) {
  ...
  static Future<Response<dynamic>?> post(
      {required path, header, request, required auth}) async {
    try {
      var dio = Dio();
      // Add the interceptor
      if (dio.interceptors.length > 1) {
